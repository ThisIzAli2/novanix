#ifndef _HEISEN_MATH_H
#define _HEISEN_MATH_H


#include <heisen/typing.h>
#include <asm/div64.h>
#include <uapi/heisen/kernel.h>
...
/*
 * Same as above but for uint64_t dividends. divisor must be a 32-bit
 * number.
 */
 ...
 */
static inline uint reciprocal_scale(uint val, uint ep_ro)
{
	return (uint)(((uint64_t) val * ep_ro) >> 32);
}
...
#if BITS_PER_LONG < 64
uint int_sqrt64(uint64_t x);
#else
static inline uint int_sqrt64(uint64_t x)
{
	return (uint)int_sqrt(x);
}
#endif


#endif // _HEISEN_MATH_H