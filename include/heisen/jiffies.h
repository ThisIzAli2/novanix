 ...
 */
extern uint64_t __cacheline_aligned_in_smp jiffies_64;
 ...
 */
static inline uint64_t jiffies_to_nsecs(const unsigned long j)
{
	return (uint64_t)jiffies_to_usecs(j) * NSEC_PER_USEC;
}

extern uint64_t jiffies64_to_nsecs(uint64_t j);
extern uint64_t jiffies64_to_msecs(uint64_t j);
